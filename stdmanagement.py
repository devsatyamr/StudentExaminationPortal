...
def get_batch():
    with open(f'C:/StudentManagementSystem_main-folder/Batch.csv','r') as f:
        reader=csv.reader(f)
...
def remove(string):
    with open(f'C:/StudentManagementSystem_main-folder/Student.csv','r+',newline='') as f:
        script=csv.reader(f)
    ...
    with open(f'{path}/Course.csv','r')as f:
        ...
        for i in range(len(rows)):
            if i==0:
                ...
                req+=[rows[i][2]]
        lst=[['Problem Solving with Python',(req[0].split('-'))[0:-1]],
...
for i in range(student_no):
    ...
    if duplicate('Student.csv',student_id,0):
        ...
        ask=input("Do you want to remove this student's data from database now is the time (Y/N) : ")
...
try:            
    ...
    print(student_no,"student's data has been successfully updated in Department.csv")
